...
func Ex1() {
	...
	var line string
	for {
		print("You can switch pages: /, /drop, /saddle, /climb.\nEnter 'stop' to stop the server and end the exercises: ")
		fmt.Scan(&line)
		if line == "stop" {
			serv.Close()
			break
...
func corner(i, j int) (float64, float64, int, error) {
	...
	// пробрасываем ошибку (3.1)
	if math.IsInf(z, 0) || math.IsNaN(z) {
		return 0, 0, 0, fmt.Errorf("бесконечный float")
	}
